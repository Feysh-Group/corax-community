...
public class XssServlet3 extends HttpServlet {
    ...
    public void writeWithEncoders(PrintWriter pw, String input1) {
        ...
        pw.write(encoded.toLowerCase() + SAFE_VALUE);  // !$cwe-79
        pw.write(StringEscapeUtils.escapeHtml(input1));  // !$cwe-79
        pw.write(ESAPI.encoder().decodeForHTML(encoded) + SAFE_VALUE);  // high priority $cwe-79
        pw.write(myEncode(input1));  // !$cwe-79
        pw.write(myDecode(encoded));  // high priority $cwe-79
        pw.write(input1.replaceAll("[\"'<>&]", ""));  // $cwe-79
    }
