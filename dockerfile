...
RUN if [ ! -f "corax-java-cli-community-$CORAX_VERSION.zip" ]; then curl -SLO "https://github.com/Feysh-Group/corax-community/releases/download/v$CORAX_VERSION/corax-java-cli-community-$CORAX_VERSION.zip"; else echo "文件已存在"; fi

RUN jar xf corax-java-cli-community-$CORAX_VERSION.zip
